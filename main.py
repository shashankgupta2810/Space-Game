...
class MainWidget(RelativeLayout):
    ...
    V_NB_LINES = 8
    V_LINES_SPACING = .4 # percentages in screen width
    vertical_lines = []

    H_NB_LINES = 15
    H_LINES_SPACING = .1  # percentages in screen width
    horizontal_lines = []
    ...
    menu_title = StringProperty("S    P    A    C    E")
    ...
    def is_desktop(self):
        if platform in ('linux', 'win', 'macosx', 'Android', 'iOS'):
            return True
        return False
    ...
    def update_horizontal_lines(self):
        start_index = - int(self.V_NB_LINES/2) + 1
        end_index = start_index + self.V_NB_LINES - 1
        xmin = self.get_line_x_index(start_index)
        xmax = self.get_line_x_index(end_index)
        for i in range(0, self.H_NB_LINES):
